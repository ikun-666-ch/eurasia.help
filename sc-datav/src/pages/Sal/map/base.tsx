...
import { useSalChartLoad } from "../panel/useSalChartLoad";
import { fetchSalesOrdersByCity } from "@/api";
...
export default function Base(props: BaseProps) {
  ...
  const { data: cityOrders } = useSalChartLoad(
  ...
  const projection = useMemo(() => {
    const centerSource = props.outlineData ?? data;
    return geoMercator()
  ...
  return (
    <group
      ...
      {regions.map((region, idx) => {
        const stats = cityStatsMap.get(region.city);
        return (
          <City
            key={idx}
            depth={depth}
            ...
            stats={
              stats
                ? { city: region.city, ...stats }
                : { city: region.city, orderCount: 0, totalAmount: 0 }
            }
          />
        );
      })}
